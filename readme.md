...
- 今は Lix インストーラーが推奨される
  - ただしそのままだと既存の homebrew が使えないので一旦アンインストールするか、 `nix-homebrew.autoMigrate = true`にする (自分が試したら、認識しなかったので一旦無視)
  - 
...
# nix-darwin に home-manager 管理を任せるのでデフォルトの ~/.config/home-manager は気にしなくて良い
# /etc/nix-darwin に dotfiles をリンクする

git clone harumaxy/dotfiles
rm -rf /etc/nix-darwin/ && ln -hsf  /etc/nix-darwin/
